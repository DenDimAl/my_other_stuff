...
int main() {
	...
	list* t = nullptr;
	list* h1 = nullptr;
	list* t1 = nullptr;//для задачи нам потребуется две исходных очереди, с помощью одной найдём минимальный элемент, с другой будем работать
	int x, min,n;
	...
	for (int i = 0; i < n; i++) {
		cin >> x;
		push_back(h, t, x);
		push_back(h1, t1, x);//заполняем обе очереди
	}
	min = h1->inf;
	while (h1) {
		if (h1->inf < min) min = h1->inf;
		h1 = h1->next;//находим минимальный элемент
	}
	list* p = h->next;//создадим ещё один указатель, с помощью которого будем искать нужные элементы 
	while (p != t) {
		if (p->inf == t->inf) insert_before(h, t, p, min);//если нашли, то вставляем перед ним минимальный
		p = p->next;
	}
	insert_before(h, t, t, min);//вставим отдельно новый элемент перед последним элементом
	if (h->inf == t->inf) push_front(h, t, min);//если в голове лежит нужный элемент, то добавим элемент в начало
	cout << "Answer is" << endl;
	print(h, t);//выводим ответ
}
